...
import {
  StickyNote, Sparkles, Send, Phone, Calendar, Clock, Check,
  Smile, Meh, Frown, Bell, CheckCircle2,
} from "lucide-react";
import { Topbar } from "@/components/Topbar";
import { Card, Select, Skeleton, Badge, Spinner, Avatar, EmptyState, SectionTitle } from "@/components/ui";
...
function NotesPage() {
  ...
  return (
    <div>
      <Topbar title="Notes & Follow-ups" subtitle="Capture notes — AI turns them into intelligence, events and reminders." />
        ...
        <div className="lg:col-span-2 space-y-4">
          <Card>
            <div className="flex items-center gap-2 mb-3">
              ...
              <div><div className="font-semibold text-slate-800">New note</div><div className="text-xs text-slate-400">AI extracts sentiment & schedules follow-ups automatically</div></div>
            </div>
            <div className="space-y-3">
              <Select value={customerId} onChange={setCustomerId} placeholder="Select customer…" options={customers.map((c) => ({ value: c.customer_id, label: `${c.full_name} · ${c.segment}` }))} />
          ...
          <div>
            ...
            <div className="space-y-3">
              {!notes ? [...Array(3)].map((_, i) => <Skeleton key={i} className="h-24" />) :
            ...
            {followUps.length === 0 ? <Card><EmptyState icon={Calendar} title="No follow-ups" subtitle="AI-detected reminders show up here." /></Card> :
              followUps.map((f, i) => (
                <motion.div key={f.followup_id} initial={{ opacity: 0, x: 12 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: Math.min(i * 0.04, 0.3) }}>
                  <Card className={cn("p-4", f.status === "Done" && "opacity-60")}>
                    <div className="flex items-start gap-3">
...
export default function Page() {
  return <Suspense fallback={<div className="p-6"><Skeleton className="h-96" /></div>}><NotesPage /></Suspense>;
}
