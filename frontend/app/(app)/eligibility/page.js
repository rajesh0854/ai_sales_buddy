...
import { ShieldCheck, ShieldX, Check, X, Sparkles, ArrowRight, Lightbulb } from "lucide-react";
...
function Eligibility() {
  ...
  return (
    <div>
      <Topbar title="Product Eligibility Checker" subtitle="Validate a recommendation against product policy before you pitch." />
      <div className="p-6 grid lg:grid-cols-3 gap-6">
        <Card className="lg:col-span-1 h-fit">
          <div className="space-y-3">
            ...
            <button onClick={check} disabled={loading || !customerId || !productId} className="btn-primary w-full py-3">
        ...
        <div className="lg:col-span-2">
          {loading ? <Card><Skeleton className="h-64" /></Card> : !result ? (
            <Card className="h-full min-h-[360px] grid place-items-center">
              <EmptyState icon={ShieldCheck} title="Run an eligibility check" subtitle="Select a customer and product to validate against the product's policy rules — with a plain-language explanation." />
            </Card>
          ) : (
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
              ...
              <Card>
                <div className="text-sm font-semibold text-slate-700 mb-3">Policy rule checks</div>
                <div className="space-y-2">
                  {result.rule_checks.map((c, i) => (
                    <motion.div key={i} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.06 }}
                      className={cn("flex items-center gap-3 rounded-xl border p-3", c.passed ? "border-emerald-100 bg-emerald-50/40" : "border-rose-100 bg-rose-50/40")}>
...
export default function Page() {
  return <Suspense fallback={<div className="p-6"><Skeleton className="h-96" /></div>}><Eligibility /></Suspense>;
}
